...
function encode_style (style_tag) {
	...
	value += (get_css_attribute('border', style_tag).split(' ')[0] == '1px'? 'T' : 'F')
	value = convert_sys_to_10(value.replaceAll('T', '1').replaceAll('F', '0'), 2)
	result += convert_10_to_sys(value)
...
function decode_style (str) {
	...
	let value = convert_10_to_sys(convert_sys_to_10(str[3]), 2).padStart(5, '0')
