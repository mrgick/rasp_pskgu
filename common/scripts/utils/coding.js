...
function fill_export_panel () {
	...
	for (key in all_REs) {
		...
		if (loaded) {
			let td = tr.children[0].children[1]
...
function export_switch_class (key) {
	...
	tr.style.opacity = tr.children[0].children[0].children[0].checked? '1' : '.3'
}
...
function export_filters () {
	...
	for (tr of tbody.children) {
		if (tr.querySelector('td > div:first-child > input').checked) {

			export_string += '!' + tr.id.replace('export_tr_', '') + ':'

			let td = tr.querySelector('td > div:last-child')
			let export_values = []
