...
function generate_weekbar (generate = 'cal') {
    ...
    let date_now = date_to_str(raw_date_now)

    document.getElementById('Weekbar').innerHTML = `
    <div id="weekbar_header" onclick="move_to_week('${main_rasp.get_table_contains(date_now).week}', '${1 + days_between(get_monday(date_now), date_now)}')">
        <div>${date_now}</div>
    </div>
    `
    let header = document.getElementById('weekbar_header')
    for (let lesson = 1; lesson <= 7; lesson++) {
        header.innerHTML += `
            <div ${document.getElementById(date_now + '-' + lesson).childElementCount > 0? 'class="WBH_have_lesson"' : ''}>
            ${lesson}
            </div>
        `
    }
